 ...
 *     Thorsten Meinl - make it thread safe
 *******************************************************************************/
...
public class ThreadsafeImageRegistry extends ImageRegistry {
    ...
    private static class OriginalImageDescriptor extends ImageDescriptor {
         ...
         */
        public OriginalImageDescriptor(final Image original, final Device originalDisplay) {
            this.original = original;
            this.originalDisplay = originalDisplay;
        }

        @Override
        public Object createResource(final Device device) throws DeviceResourceException {
        ...
        @Override
        public void destroyResource(final Object toDispose) {
    ...
    private void putEntry(final String key, final Entry entry) {
        getTable().put(key, entry);
    }

    private Map<String, Entry> getTable() {
        if (table == null) {
            table = new ConcurrentHashMap<String, Entry>(16);
        }
        return table;
    }
