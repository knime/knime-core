...
public class FallbackIteratorTest {
    ...
    public void testIdentity() throws Exception {
        ...
        try (DataRowCursor cursor = fullTable.cursor(); CloseableRowIterator it = fullTable.iterator()) {
            assertEquals(cursor.getNumValues(), fullTable.getDataTableSpec().getNumColumns());
            while (cursor.forward()) {
                assertTrue(it.hasNext());
                DataRow row = it.next();
                for (int i = 0; i < cursor.getNumValues(); i++) {
