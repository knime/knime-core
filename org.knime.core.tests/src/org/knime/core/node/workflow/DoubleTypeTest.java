...
/**
 * Unit tests for {@link DoubleType}.
 *
...
public class DoubleTypeTest extends AbstractSimpleVariableTypeTest<Double> {
    ...
    public DoubleTypeTest() {
        super(SharedIcons.FLOWVAR_DOUBLE.get(), Double.class, DoubleType.INSTANCE, OVERWRITABLE, CREATION_COMPATIBLE,
            Sets.newHashSet(DoubleType.INSTANCE, DoubleArrayType.INSTANCE, StringType.INSTANCE,
                StringArrayType.INSTANCE));
    }

    @Override
    public void testLoadValue() throws InvalidSettingsException {
        when(readSettings.getDouble(CFG_VALUE)).thenReturn(1.0);
        assertEquals(Double.valueOf(1.0), m_testInstance.loadValue(readSettings).get());
    }

    @Override
    public void testSaveValue() {
        m_testInstance.saveValue(writeSettings, m_testInstance.newValue(1.0));
        Mockito.verify(writeSettings).addDouble(CFG_VALUE, 1.0);
    }
