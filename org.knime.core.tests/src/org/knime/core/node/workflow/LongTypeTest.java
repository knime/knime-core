...
public class LongTypeTest extends AbstractSimpleVariableTypeTest<Long> {
     ...
     */
    public LongTypeTest() {
        super(SharedIcons.FLOWVAR_LONG.get(), Long.class, LongType.INSTANCE, OVERWRITABLE, CREATION_COMPATIBLE,
            Sets.newHashSet(LongType.INSTANCE, LongArrayType.INSTANCE, DoubleType.INSTANCE, DoubleArrayType.INSTANCE,
                StringType.INSTANCE, StringArrayType.INSTANCE));
    }

    @Override
    public void testLoadValue() throws InvalidSettingsException {
        when(readSettings.getLong(CFG_VALUE)).thenReturn(1L);
        assertEquals(Long.valueOf(1), m_testInstance.loadValue(readSettings).get());
    }

    @Override
    public void testSaveValue() {
        m_testInstance.saveValue(writeSettings, m_testInstance.newValue(1L));
        verify(writeSettings).addLong(CFG_VALUE, 1);
    }
