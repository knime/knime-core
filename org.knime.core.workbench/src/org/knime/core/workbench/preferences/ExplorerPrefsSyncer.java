...
public final class ExplorerPrefsSyncer implements IPreferenceChangeListener, INodeChangeListener {

    private static final Logger LOGGER = LoggerFactory.getLogger(ExplorerPrefsSyncer.class);
    ...
    private synchronized void updateSettings(final List<MountSettings> oldValues, final List<MountSettings> newValues) {
        ...
        for (MountSettings ms : oldSettings) {
            boolean successful = WorkbenchMountTable.unmount(ms.getMountID());
            if (!successful) {
                // most likely mount point was not present to begin with
                LOGGER.debug("Mount point \"{}\" could not be unmounted.", ms.getDisplayName());
            }
        }
