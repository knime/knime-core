...
final class MountPointPreferencesTest {
    ...
    void testMountPointLoading() throws Exception {
        ...
        initialSettings.removeIf(m -> CoreConstants.KNIME_HUB_MOUNT_ID.equals(m.getMountID()));

        int numberOfSettings = initialSettings.size();

        List<String> initialMountIDs = initialSettings.stream().map(ms -> ms.getMountID()).toList();
        ...
        // required when running tests inside Eclipse with server space available
        modifiedSettings.removeIf(m -> CoreConstants.KNIME_HUB_MOUNT_ID.equals(m.getMountID()));


        List<String> newMountIDs= modifiedSettings.stream().map(ms -> ms.getMountID()).toList();
