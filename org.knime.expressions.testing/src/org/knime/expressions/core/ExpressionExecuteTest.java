...
public class ExpressionExecuteTest {
	...
	public void returnTest() {
		...
		try {
			function = Expressions.wrap(exp, null, null, DataType.getType(DoubleCell.class));
	...
	public void columnAccessTest() {
		...
		try {
			function = Expressions.wrap(expr, new DataColumnSpec[] { col1, col2 }, null,
					DataType.getType(IntCell.class));

			IntCell result = (IntCell) function.apply(new DefaultScriptRowInput(row, 0, 0));

			assertEquals(5, result.getIntValue());

			result = (IntCell) function.apply(new DefaultScriptRowInput(row2, 0, 0));
	...
	public void testFlowVariableAccess() {
		...
		try {
			function = Expressions.wrap(expr, null, new FlowVariable[] { var1, var2 }, DataType.getType(IntCell.class));
	 ...
	 * {@link org.knime.expressions.base.sets.logical.And} and
	 * {@link org.knime.expressions.base.sets.logical.Or}.
	 */
	@Test
	public void functionCallTest() {
		...
		try {
			...
			BooleanCell result = (BooleanCell) function.apply(new DefaultScriptRowInput(row, 0, 0));

			assertFalse(result.getBooleanValue());

			result = (BooleanCell) function.apply(new DefaultScriptRowInput(row2, 0, 0));

			assertTrue(result.getBooleanValue());

			result = (BooleanCell) function.apply(new DefaultScriptRowInput(row3, 0, 0));
