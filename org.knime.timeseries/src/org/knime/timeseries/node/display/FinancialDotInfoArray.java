 ...
 * --------------------------------------------------------------------- *
 * 
 * Created 22-02-2007 (Rosaria Silipo)
 */
...
/**
 * Class that holds all <code>FinancialDotInfo</code>s. 
 * Just sort them after the modifications are done. Some functions
 * of this class depend on the fact that the Array is sorted.
 * This is the same as a DotInfoArray only that 4 numbers are stored 
 * for each point rather than 2: high price, low peice, open price, close price.
 * These for numbers will be used to draw the points 
 * from the FinancialShapeFactory
 * 
 * @author Rosaria Silipo
 */
public class FinancialDotInfoArray {
...
//                DotInfoArray.class);
    ...
    private FinancialDotInfo[] m_dots;
     ...
     */
    public FinancialDotInfoArray(final int size) {
        m_dots = new FinancialDotInfo[size];
    }
     ...
     */
    public FinancialDotInfoArray(final FinancialDotInfo[] dots) {
        m_dots = dots;
    }
    ...
    public void sort(final int sortPolicy) {
        ...
        Comparator<FinancialDotInfo> comparator = new CoordComp(sortPolicy);
     ...
     */
    public FinancialDotInfo[] getDots() {
        return m_dots;
    }
     ...
     */
    public List<FinancialDotInfo> getDotsAt(final int x, 
                                   final int y, 
                                   final int dotSize) {
        List<FinancialDotInfo> result = new ArrayList<FinancialDotInfo>();
        for (int i = 0; i < m_dots.length; i++) {
            FinancialDotInfo dot = m_dots[i];
            int size = (int)Math.round(dotSize * (dot.getSize() + 0.5));
            int halfSize = (int)Math.round(size / 2.0);
            Rectangle rect = new Rectangle(
                    dot.getXCoord() - halfSize,
                    dot.getHighPrice() - dot.getLowPrice(),
                    size, dot.getHighPrice() - dot.getLowPrice());
...
//        return getDotsContainedIn(x, y, x, y, dotSize);
     ...
     */
    public List<FinancialDotInfo> getDotsContainedIn(final int x1, final int y1,
        ...
        ArrayList<FinancialDotInfo> result = new ArrayList<FinancialDotInfo>();
        ...
        for (int i = 0; i < m_dots.length; i++) {
            ...
            Rectangle dotRect = new Rectangle(
                    ...
                    m_dots[i].getHighPrice(),
                    size, 
                    m_dots[i].getLowPrice() - m_dots[i].getHighPrice());
    ...
    private static class CoordComp 
                    implements Comparator<FinancialDotInfo>, Serializable {
        ...
        public int compare(final FinancialDotInfo o1, final FinancialDotInfo o2) {
            ...
            if (m_sortPolicy == FinancialDotInfoArray.X_THEN_Y_SORT_POLICY) {
                firstOrderCriteria1 = o1.getXCoord();
                firstOrderCriteria2 = o2.getXCoord();
                secondOrderCriteria1 = o1.getHighPrice();
                secondOrderCriteria2 = o2.getHighPrice();
            } else if (m_sortPolicy == FinancialDotInfoArray.Y_THEN_X_SORT_POLICY) {
                firstOrderCriteria1 = o1.getHighPrice();
                firstOrderCriteria2 = o2.getHighPrice();
            ...
            throw new IllegalStateException("During sorting DotInfo a state "
                    + "was reached which should never occur.");
        }
    }
