 ...
 *
 * History
 *   24 Nov 2024 (Bernd Wiswedel, KNIME GmbH, Konstanz, Germany): created
 */
...
public final class BugAP23528_WorkflowLoad_FolderNames_CaseIgnorant extends WorkflowTestCase {
    ...
    @Test
    public void testLoadWorkflowWithWrongFolderName() throws Exception {
    	final WorkflowLoadResult loadResult = loadWorkflow(m_copiedWorkflowFolder, new ExecutionMonitor());
    	setManager(loadResult.getWorkflowManager());
    	assertThat("max load error level", loadResult.getType(), is(LoadResultEntryType.Ok));
    	NodeID wkfID = loadResult.getWorkflowManager().getID();
    	NodeID tableCreator_1 = wkfID.createChild(1);
    	NodeID diffChecker_4 = wkfID.createChild(4);
