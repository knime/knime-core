...
public class BugAP19479_Component_Encryption_Cipher47 extends WorkflowTestCase {
     ...
     * This should fail for every version <= 4.6.1.
     */
    @Test
    public void load47LockedComponentIn46Test() throws Exception {
        ...
        assertTrue("Loading should produce errors.", result.hasErrors());
        assertTrue("Error message should indicate that cipher could not be initialized.",
                result.getFilteredError("", LoadResultEntryType.Error).contains("Unable to init cipher"));
        ...
        final var wfm2 = ((SubNodeContainer) locked).getWorkflowManager();
        assertEquals("Only input and output should be present.", 2, wfm2.getNodeContainers().size());
    }
