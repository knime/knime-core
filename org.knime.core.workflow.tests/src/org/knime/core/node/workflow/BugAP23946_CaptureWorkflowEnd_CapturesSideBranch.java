...
public class BugAP23946_CaptureWorkflowEnd_CapturesSideBranch extends WorkflowTestCase {
    ...
    private WorkflowLoadResult initWorkflowFromTemp() throws Exception {
        ...
        getLogger().info("BERND: Loading workflow from: " + workflow.getAbsolutePath());
    ...
    private void logFilesRecursively(File dir, int depth) {
        if (dir == null || !dir.exists()) {
            getLogger().info("Directory does not exist: " + dir);
            return;
        }
        String indent = "  ".repeat(depth);
        for (File file : dir.listFiles()) {
            if (file.isDirectory()) {
                getLogger().info(indent + "[DIR] " + file.getName());
                logFilesRecursively(file, depth + 1);
            } else {
                getLogger().info(indent + file.getName());
            }
        }
    }
    ...
    public void testWriteWorkflowAfterRemovingSideBranchWithAdditionalCaptureWorkflowGroup() throws Exception {
        ...
        getLogger().info("BERND: SAVED LOCATION: " + generatedWorkflow.getAbsolutePath());
        getLogger().info("BERND: Listing all files in directory: " + m_testCaseDir.getAbsolutePath());
        logFilesRecursively(m_testCaseDir, 0);
