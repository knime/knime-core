...
public class BugAP13760_TemporaryFilesInLoopNotDeletedOnClear extends WorkflowTestCase {
	...
	public void testExecutePlain() throws Exception {
		...
		final File tempDir = manager.getContextV2().getExecutorInfo().getTempFolder().toFile();
		Thread.sleep(100); // give the file-in-background-deletion some time to do its work
		assertEquals(0, countFilesInDirectory(tempDir), String.format(
				"Files remaining in workflow temp directory after clear: %s.", Arrays.toString(tempDir.list())));
	}
