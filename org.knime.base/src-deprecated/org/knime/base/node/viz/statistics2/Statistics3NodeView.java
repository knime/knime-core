...
public class Statistics3NodeView extends NodeView<Statistics3NodeModel> {
     ...
     */
    Statistics3NodeView(final Statistics3NodeModel model) {
        super(model);
    ...
    private void refreshStastics() {
        Statistics3NodeModel myModel = getNodeModel();
    ...
    private void refreshNominalValues() {
        ...
        final List<Map<DataCell, Integer>> nominals = myModel.getNominals();
        ...
        int[] missings = myModel.getNumMissingValues();
        for (int i = 0; i < columnNames.length; i++) {
        	if (nominals.get(i) != null) {
