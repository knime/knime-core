...
public final class LogRegLearner {
    private static final NodeLogger LOGGER = NodeLogger.getLogger(LogRegLearner.class);
     ...
     */
    public LogRegLearner(final PortObjectSpec[] specs, final LogRegLearnerSettings settings)
        throws InvalidSettingsException {
        m_settings = settings;
        DataTableSpec dataSpec = (DataTableSpec)specs[0];
        init(dataSpec, Collections.<String> emptySet());
    }
     ...
     */
    public LogisticRegressionContent execute(final PortObject[] portObjects, final ExecutionContext exec)
        throws Exception {
        ...
        BufferedDataTable dataTable =
            recalcDomainForTargetAndLearningFields(data, exec.createSubExecutionContext(calcDomainTime));
    ...
    private void init(final DataTableSpec inSpec, final Set<String> exclude) throws InvalidSettingsException {
        ...
        if (null != targetColSpec) {
            ...
            m_learner = new Learner(m_pmmlOutSpec, m_specialColumns, m_settings.getTargetReferenceCategory(),
