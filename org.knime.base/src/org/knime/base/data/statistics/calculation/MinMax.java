...
public class MinMax extends Statistic {
    ...
    private Comparator<DataCell>[] m_comparators;

    private DataCell[] m_min;

    private DataCell[] m_max;

    /**
     * @param columns to calculate the min/max values
     */
    public MinMax(final String... columns) {
    ...
    protected void init(final DataTableSpec spec, final int size) {
        m_min = new DataCell[size];
        m_max = new DataCell[size];
        m_comparators = new Comparator[size];
        for (int i = 0; i < size; i++) {
            m_comparators[i] = spec.getColumnSpec(getIndices()[i]).getType().getComparator();
            m_min[i] = DataType.getMissingCell();
            m_max[i] = DataType.getMissingCell();
        }
    }
     ...
     */
    public DataCell getMin(final String column) {
     ...
     */
    public DataCell getMax(final String column) {
