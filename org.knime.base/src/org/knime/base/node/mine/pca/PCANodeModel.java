...
public class PCANodeModel extends NodeModel {
	...
	private final SettingsModel[] m_settingsModels = { m_inputColumns,
			m_removeOriginalCols, m_failOnMissingValues, m_dimSelection };
	...
	protected DataTableSpec[] configure(final DataTableSpec[] inSpecs)
		...
		int dim = m_dimSelection
				.getNeededDimensions(m_inputColumnIndices.length);
		if (dim == -1) {
	...
	protected PortObject[] execute(final PortObject[] inData,
		...
		final int dimensions = m_dimSelection
				.getNeededDimensions(m_inputColumnIndices.length);
	...
	public static BufferedDataTable createDecompositionOutputTable(
			final ExecutionContext exec, EigenvalueDecomposition evd,
			String[] columnNames) throws CanceledExecutionException {
		List<EigenValue> sortedEV = EigenValue.createSortedList(evd.getV()
				.getArray(), EigenValue.extractEVVector(evd));
	...
	protected static DataCell[] convertInputRow(final Matrix eigenvectors,
		...
		if (missingValues) {
			...
			for (int i = 0; i < rowVec.length; i++) {
				...
				rowVec[i][0] = ((DoubleValue) row
						.getCell(inputColumnIndices[i])).getDoubleValue()
						- means[i];

			}
			final double[][] newRow = new Matrix(rowVec).transpose()
