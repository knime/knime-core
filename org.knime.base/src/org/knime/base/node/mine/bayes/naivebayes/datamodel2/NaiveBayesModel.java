...
public class NaiveBayesModel {
    ...
    public double[] getClassProbabilities(final String[] attributeNames, final DataRow row,
        ...
        for (int i = 0, length = classValues.size(); i < length; i++) {
            probs[i] = getClassProbability(attributeNames, row, classValues.get(i));
            sum += probs[i];
        }
        if (sum == 0) {
     ...
     * given row.
     * @param attributeNames the attribute names in the same order
    ...
    public String getMostLikelyClass(final String[] attributeNames, final DataRow row) {
        ...
        for (final String classValue : classValues) {
            final double classProbability = getClassProbability(attributeNames, row, classValue);
    ...
    /**
     * Returns the {@link DataCell} of the class with the highest probability for the
     * given row.
     * @param attributeNames the attribute names in the same order
     * they appear in the given data row
     * @param row the row with the attributes in the same order like in the
     * training data table
     * @return the class attribute {@link DataCell} with the highest probability for the given
     * attribute values.
     */
    ...
    private static DataCell createPredictedClassCell(final DataType dataType, final String val) {
        if (dataType.isCompatible(BooleanValue.class)) {
            return BooleanCell.get(Boolean.parseBoolean(val));
            ...
            return new DoubleCell(Double.parseDouble(val));
        }
