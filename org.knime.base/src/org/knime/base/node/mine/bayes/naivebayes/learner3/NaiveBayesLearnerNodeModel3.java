...
import org.knime.base.node.mine.bayes.naivebayes.datamodel3.AttributeModel;
...
final class NaiveBayesLearnerNodeModel3 extends NodeModel {
    ...
    static final double MIN_SD_VALUE_DEF = 1e-4;
    ...
    private final SettingsModelDouble m_minSdThreshold = createMinSdEpsilonModel();
    ...
    /**
     * Creates the minimum standard deviation epsilon model.
     *
     * @return the minimum standard deviation epsilon model
     */
    static SettingsModelDoubleBounded createMinSdEpsilonModel() {
        return new SettingsModelDoubleBounded(CFG_MIN_SD_THRESHOLD_KEY, MIN_SD_THRESHOLD_DEF, 0, Double.MAX_VALUE);
    }
     ...
     * Constructor.
     */
    protected NaiveBayesLearnerNodeModel3() {
    ...
    protected PortObjectSpec[] configure(final PortObjectSpec[] inSpecs) throws InvalidSettingsException {
        ...
        for (final DataColumnSpec colSpec : tableSpec) {
            final AttributeModel model = NaiveBayesModel.getCompatibleModel(colSpec, classColumn, maxNoOfNominalVals,
                ...
                m_minSdValue.getDoubleValue(), m_minSdThreshold.getDoubleValue());
            if (model == null) {
                ...
                if (domain != null && domain.getValues() != null) {
                    if (domain.getValues().size() > maxNoOfNominalVals) {
                        ...
                        if (colSpec.getName().equals(classColumn)) {
                            ...
                            throw new InvalidSettingsException("Class column domain contains too many unique values"
                                + " (count: " + domain.getValues().size() + ")");
                        }
        ...
        final PMMLPortObjectSpec pmmlSpec = createPMMLSpec(tableSpec, learnCols, classColumn);
        return new PortObjectSpec[]{pmmlSpec,
            NaiveBayesModel.createStatisticsTableSpec(classColSpec.getType(), m_ignoreMissingVals.getBooleanValue())};
    }
    ...
    protected PortObject[] execute(final PortObject[] inData, final ExecutionContext exec)
        ...
        LOGGER.debug("Entering execute of " + NaiveBayesLearnerNodeModel3.class.getName());
        ...
        m_model = new NaiveBayesModel(trainingTable, m_classifyColumnName.getStringValue(), exec, maxNoOfNomVals,
            ...
            m_minSdThreshold.getDoubleValue());
        ...
        LOGGER.debug("Exiting execute of " + NaiveBayesLearnerNodeModel3.class.getName());

        // handle the optional PMML input
        final DataTableSpec tableSpec = trainingTable.getSpec();
        final PMMLPortObjectSpec outPortSpec =
            createPMMLSpec(tableSpec, m_model.getPMMLLearningCols(), m_model.getClassColumnName());
        final PMMLPortObject outPMMLPort = new PMMLPortObject(outPortSpec, null, tableSpec);
    ...
    private static PMMLPortObjectSpec createPMMLSpec(final DataTableSpec tableSpec, final List<String> learnCols,
        final String classColumn) {
        final PMMLPortObjectSpecCreator pmmlSpecCreator = new PMMLPortObjectSpecCreator(tableSpec);
    ...
    protected void saveSettingsTo(final NodeSettingsWO settings) {
        ...
        m_minSdThreshold.saveSettingsTo(settings);
    }
    ...
    protected void loadValidatedSettingsFrom(final NodeSettingsRO settings) throws InvalidSettingsException {
        ...
        /* (3.7.1) this breaks backwards compatibility (different results for some data) since 0
         * was allowed and the default before.
        ...
        if (settings.containsKey(CFG_MIN_SD_VALUE_KEY)) {
            ...
            m_minSdValue.setDoubleValue(MIN_SD_VALUE_DEF);
        }
        // (3.7.1) ensure backwards compatibility
        if (settings.containsKey(CFG_MIN_SD_THRESHOLD_KEY)) {
            m_minSdThreshold.loadSettingsFrom(settings);
        } else {
            // ensure backwards compatibility in case we ever decide to change the default
        	m_minSdThreshold.setDoubleValue(0.0);
        }
    }
