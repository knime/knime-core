 ...
 */
final class NaiveBayesCellFactory implements AppendedCellFactory, CellFactory {

    /**
     * The <code>NaiveBayesModel</code> which holds all necessary information to calculate the probability for new
     * records.
     */
     ...
     *
     * @param model the <code>NaiveBayesModel</code> which holds all necessary information to calculate the probability
     *            for new records.
     * @param columnName the prediction column name
     * @param tableSpec the basic table specification
     * @param inclClassProbVals if the probability per class instance should be appended as columns
     * @param suffix the suffix for the probability columns
     */
    NaiveBayesCellFactory(final NaiveBayesModel model, final String columnName, final DataTableSpec tableSpec,
    ...
    /**
     * Creates the column specification of the result columns and returns them in the order they should be appended to
     * the original table specification.
     *
     * @param model the {@link NaiveBayesModel} to use
     * @param predictionColName the name of the prediction column
     * @param inSpec the <code>DataTableSpec</code> of the input data to check if the winner column name already exists
     * @param inclClassProbVals if the probability values should be displayed
     * @param suffix the suffix for the probability columns
     * @return <code>DataColumnSpec[]</code> with the column specifications of the result columns
     */
    private static DataColumnSpec[] createResultColSpecs(final NaiveBayesModel model, final String predictionColName,
        final DataTableSpec inSpec, final boolean inclClassProbVals, final String suffix) {
     ...
     * @return <code>DataColumnSpec[]</code> with the column specifications of the result columns
     */
    static DataColumnSpec createResultColSpecs(final String classColumnName, final DataType classType,
        final DataTableSpec inSpec, final boolean inclClassProbVals) {
        if (inclClassProbVals) {
            return null;
        }
    ...
    private static DataColumnSpec createPredictedClassColSpec(final String classColumnName, final DataType classType,
        ...
        final DataType pmmlConformDataType =
            PMMLDataDictionaryTranslator.getKNIMEDataType(PMMLDataDictionaryTranslator.getPMMLDataType(classType));
    ...
    /**
     * @return the specification of the result columns in the order they should be append at the end of the original
     *         table specification
     */
