 ...
 */
package org.knime.base.node.preproc.rename2;
 ...
 */
public class Rename2NodeDialogPane extends NodeDialogPane {

    private static final RenameColumn2Panel DUMMY_PANEL = new RenameColumn2Panel(new RenameColumn2Setting("DUMMY"),
        DataColumnSpecListCellRenderer.createInvalidSpec("DUMMY"));

    private final Map<DataColumnSpec, RenameColumn2Setting> m_columnToSettings;
     ...
     */
    public Rename2NodeDialogPane() {
        super();

        m_columnToSettings = new LinkedHashMap<DataColumnSpec, RenameColumn2Setting>();
                ...
                switch (searchEvent.getType()) {
                    ...
                    case DELETION:
                        for (DataColumnSpec spec : m_searchableListPanel.getSelectedColumns()) {
                            ...
                            RenameColumn2Setting renameColumnSetting = m_columnToSettings.get(spec);
                            if (renameColumnSetting != null) {
                                int indexIndividualIndex = getIndexIndividualIndex(m_columnToSettings.get(spec));
                                removeFromIndividualPanel((RenameColumn2Panel)m_individualsPanel
    ...
    protected void loadSettingsFrom(final NodeSettingsRO settings, final DataTableSpec[] specs)
        ...
        try {
            ...
            // NodeSetting
            subSettings = settings.getNodeSettings(Rename2NodeModel.CFG_SUB_CONFIG);
        ...
        if (subSettings != null) {
            ...
            for (String id : subSettings) {
                ...
                try {
                    ...
                    nameForSettings = idSettings.getString(RenameColumn2Setting.CFG_OLD_COLNAME);
                ...
                final RenameColumn2Setting renameColumnSetting =
                    orgSpec == null ? new RenameColumn2Setting(nameForSettings) : new RenameColumn2Setting(orgSpec);
        ...
        for (Map.Entry<DataColumnSpec, RenameColumn2Setting> entries : m_columnToSettings.entrySet()) {
            addToIndividualPanel(new RenameColumn2Panel(entries.getValue(), entries.getKey()));
        }
    ...
    protected void saveSettingsTo(final NodeSettingsWO settings) throws InvalidSettingsException {
        NodeSettingsWO subSettings = settings.addNodeSettings(Rename2NodeModel.CFG_SUB_CONFIG);
        Map<String, RenameColumn2Setting> duplicateHash = new HashMap<String, RenameColumn2Setting>();
        ...
        for (RenameColumn2Setting colSet : m_columnToSettings.values()) {
        ...
        for (RenameColumn2Setting colSet : m_columnToSettings.values()) {
            ...
            RenameColumn2Setting put = duplicateHash.put(newName, colSet);
    ...
    private void createAndAddRenamePanelSetting() {
        ...
        if (selected != null && !m_columnToSettings.containsKey(selected)) {
            RenameColumn2Setting renameColumnSetting = new RenameColumn2Setting(selected);
            m_columnToSettings.put(selected, renameColumnSetting);
            addToIndividualPanel(new RenameColumn2Panel(renameColumnSetting, selected));
        }
    }
    ...
    private void clearBorders() {
        ...
        for (int i = 0; i < m_individualsPanel.getComponentCount(); i++) {
            RenameColumn2Panel panel = ((RenameColumn2Panel)m_individualsPanel.getComponent(i));
    ...
    private void addToErrornousColNames(final String colName, final int... individualIndices) {
        ...
        for (int i : individualIndices) {
            ((RenameColumn2Panel)m_individualsPanel.getComponent(i)).setBorder(BorderFactory.createLineBorder(Color.RED,
                1));
        }
    }
     ...
     *         <code>-1</code>.
     */
    private int getIndexIndividualIndex(final RenameColumn2Setting colSet) {
        for (int i = 0; i < m_individualsPanel.getComponentCount(); i++) {
            RenameColumn2Panel component = (RenameColumn2Panel)m_individualsPanel.getComponent(i);
    ...
    private void removeFromIndividualPanel(final RenameColumn2Panel panel) {
    ...
    private void addToIndividualPanel(final RenameColumn2Panel panel) {
        panel.addPropertyChangeListener(RenameColumn2Panel.REMOVE_ACTION, new PropertyChangeListener() {
            /** {@inheritDoc} */
            @Override
