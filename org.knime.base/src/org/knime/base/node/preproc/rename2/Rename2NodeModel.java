 ...
 */
public class Rename2NodeModel extends NodeModel {
    /**
    ...
    private static final NodeLogger LOGGER = NodeLogger.getLogger(Rename2NodeModel.class);

    /** contains settings for each individual column. */
    private RenameColumn2Setting[] m_settings;

    /**
     * Create new model, does nothing fancy.
     */
    public Rename2NodeModel() {
        super(1, 1);
    }
    ...
    protected void saveSettingsTo(final NodeSettingsWO settings) {
        ...
        if (m_settings != null) {
            for (int i = 0; i < m_settings.length; i++) {
                RenameColumn2Setting set = m_settings[i];
    ...
    private RenameColumn2Setting[] load(final NodeSettingsRO settings) throws InvalidSettingsException {
        NodeSettingsRO subSettings = settings.getNodeSettings(CFG_SUB_CONFIG);
        ArrayList<RenameColumn2Setting> result = new ArrayList<RenameColumn2Setting>();
        for (String identifier : subSettings) {
            NodeSettingsRO col = subSettings.getNodeSettings(identifier);
            result.add(RenameColumn2Setting.createFrom(col));
        }
        return result.toArray(new RenameColumn2Setting[0]);
    }
    ...
    protected DataTableSpec[] configure(final DataTableSpec[] inSpecs) throws InvalidSettingsException {
        ...
        List<RenameColumn2Setting> renameSettings =
            m_settings == null ? new ArrayList<RenameColumn2Setting>() : new ArrayList<RenameColumn2Setting>(
                Arrays.asList(m_settings));

        for (int i = 0; i < colSpecs.length; i++) {
            DataColumnSpec current = inSpec.getColumnSpec(i);
            String name = current.getName();
            RenameColumn2Setting set = findAndRemoveSettings(name, renameSettings);
        ...
        if (!renameSettings.isEmpty()) {
            ...
            for (RenameColumn2Setting setting : renameSettings) {
     ...
     */
    private RenameColumn2Setting findAndRemoveSettings(final String colName,
        final List<RenameColumn2Setting> renameSettings) {

        Iterator<RenameColumn2Setting> listIterator = renameSettings.iterator();
        while (listIterator.hasNext()) {
            RenameColumn2Setting set = listIterator.next();
