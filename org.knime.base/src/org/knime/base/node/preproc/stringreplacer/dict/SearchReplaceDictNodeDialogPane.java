...
/**
 * Dialog for Search & Replace (Dictionary) node. Contains a column selection panel for target column selection, a file
...
final class SearchReplaceDictNodeDialogPane extends NodeDialogPane {
    ...
    public SearchReplaceDictNodeDialogPane() {
        m_targetColBox = new ColumnSelectionComboxBox((Border)null, StringValue.class);
        FlowVariableModel fvm = createFlowVariableModel("flowVariableModelSettings", Type.STRING);
        m_fileChooserPanel =
            new FilesHistoryPanel(fvm, "string_replace_dict", LocationValidation.FileInput, new String[]{".txt"});
        ...
        m_delimiterField.addKeyListener(new KeyAdapter() {
            ...
            public void keyTyped(final KeyEvent e) {
                ...
                if ((newText.length() == 1 && !newText.startsWith("\\")) || newText.length() > 1) {
                    m_delimiterField.selectAll();
                }
            }
        });
    ...
    protected void loadSettingsFrom(final NodeSettingsRO settings, final DataTableSpec[] specs)
        ...
        String targetCol = settings.getString(SearchReplaceDictNodeModel.CFG_TARGET_COLUMN, null);
        String dictLocation = settings.getString(SearchReplaceDictNodeModel.CFG_DICT_LOCATION, null);
        char delimChar = settings.getChar(SearchReplaceDictNodeModel.CFG_DELIMITER_IN_DICT, ',');
        String appendCol = settings.getString(SearchReplaceDictNodeModel.CFG_APPEND_COLUMN, null);
    ...
    protected void saveSettingsTo(final NodeSettingsWO settings) throws InvalidSettingsException {
        ...
        if (delimString.charAt(0) == '\\') {
            switch (delimString.charAt(1)) {
                ...
                case 't':
                    ...
                    break;
                default:
                    throw new InvalidSettingsException("\"" + delimString + "\" is not a valid delimiter, "
        ...
        String appendCol = m_appendChecker.isSelected() ? m_appendTextField.getText() : null;
        settings.addString(SearchReplaceDictNodeModel.CFG_TARGET_COLUMN, targetCol);
        settings.addString(SearchReplaceDictNodeModel.CFG_DICT_LOCATION, dictLoc);
        settings.addString(SearchReplaceDictNodeModel.CFG_APPEND_COLUMN, appendCol);
        settings.addChar(SearchReplaceDictNodeModel.CFG_DELIMITER_IN_DICT, delim);
    }
