 ...
 */
public class Many2OneCol2NodeDialog extends DefaultNodeSettingsPane {

    // new column name
    private DialogComponentString m_appendedColumnName;

    // column filter to condense
    private DialogComponentColumnFilter2 m_columns2Condense;
    ...
    /**
     * Adds textfield for new column name, column filter for columns to condense, include method (max or min value in
     * row, or 0,1, or regexp pattern), textfield for regexp pattern (if selected), boolean keep columns, boolean allow
     * multiple columns match in one row.
     *
     */
    public Many2OneCol2NodeDialog() {
        ...
        m_includeMethod = new DialogComponentStringSelection(includeModel, "Include method", values);
        final SettingsModelString patternModel =
            new SettingsModelString(Many2OneCol2PMMLNodeModel.RECOGNICTION_REGEX, "[^0]*");
        // initially disable/enable
        patternModel.setEnabled(includeModel.getStringValue().equals(
        ...
        m_keepColumns =
            new DialogComponentBoolean(new SettingsModelBoolean(Many2OneCol2PMMLNodeModel.KEEP_COLS, true),
