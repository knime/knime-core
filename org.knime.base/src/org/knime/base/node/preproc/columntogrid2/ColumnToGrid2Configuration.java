 ...
 */
final class ColumnToGrid2Configuration {
     ...
     */
    public ColumnToGrid2Configuration(final DataTableSpec spec)
        throws InvalidSettingsException {
        m_colCount = DEF_COL_COUNT;
        m_filterConfig = ColumnToGrid2NodeModel.createDCSFilterConfiguration();
        String[] includes = autoGuessIncludeColumns(spec);
        if (includes == null) {
            throw new InvalidSettingsException(
                    "No reasonable auto-configuration possible");
        } else {
            m_filterConfig.loadDefaults(includes, null, EnforceOption.EnforceInclusion);
        }
    }

    /** Default constructor, inits field empty. */
    public ColumnToGrid2Configuration() {
        // nothing to do
    }
     ...
     */
    public void setFilterConfiguration(final DataColumnSpecFilterConfiguration filterConfig) {
        m_filterConfig = filterConfig;
    }

    /**
     * @return filter configuration for DataColumnSpecFilterPanel
     */
