...
class RoundDoubleNodeModel extends NodeModel {
    ...
    private SettingsModelFilterString m_filterDoubleColModel = 
    ...
    throws InvalidSettingsException {
        ...
        if (noDoubleCols <= 0) {
            throw new InvalidSettingsException(
                    "No columns containing double values in input table!");
        }
        // check if all included columns are available in the spec
        List<String> includedColNames = m_filterDoubleColModel.getIncludeList();
        for (String colName : includedColNames) {
            if (!dataSpec.containsName(colName)) {
        ...
        int[] includedColIndices = getIncludedColIndices(dataSpec);
        ...
        DataColumnSpec[] newColsSpecs = getNewColSpecs(append, colSuffix, 
                outputAsString, includedColNames, dataSpec);
    ...
    private static final DataColumnSpec[] getNewColSpecs(final boolean append, 
            ...
            final List<String> colNamesToRound, final DataTableSpec origInSpec) 
    {
        DataColumnSpec[] appColumnSpecs = 
            new DataColumnSpec[colNamesToRound.size()];
        int i = 0;
    ...
    protected void validateSettings(final NodeSettingsRO settings)
        ...
        List<String> selectedCols = ((SettingsModelFilterString)
                m_filterDoubleColModel.createCloneWithValidatedValue(settings))
                .getIncludeList();
