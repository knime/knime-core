 ...
 * @since 2.8
 */
public class Many2OneCol2PMMLNodeModel extends NodeModel {
    ...
    /**
     * The name of the settings tag which holds <code>true</code> if the user wants have the selected condensing columns
     * removed as <code>boolean</code>.
     */
    public static final String KEEP_COLS = "many2OneRemoveCols";

    /**
     * The name of the settings tag which holds <code>true</code> if the user wants that the node handles multiple
     * occurrences as <code>boolean</code>.
     */
     ...
     * @return the settings model
     */
    static SettingsModelColumnFilter2 createIncludedColumnsSettingsModel() {
        return new SettingsModelColumnFilter2(SELECTED_COLS);
    }
    ...
    private final SettingsModelColumnFilter2 m_includedColumns = createIncludedColumnsSettingsModel();
    ...
    public Many2OneCol2PMMLNodeModel(final boolean pmmlEnabled) {
        ...
        super(pmmlEnabled ? new PortType[]{BufferedDataTable.TYPE, new PortType(PMMLPortObject.class, true)}
            : new PortType[]{BufferedDataTable.TYPE}, pmmlEnabled ? new PortType[]{BufferedDataTable.TYPE,
            new PortType(PMMLPortObject.class, true)} : new PortType[]{BufferedDataTable.TYPE});
        ...
        m_includeMethod.addChangeListener(new ChangeListener() {
            ...
            public void stateChanged(final ChangeEvent e) {
                m_pattern.setEnabled(m_includeMethod.getStringValue().equals(IncludeMethod.RegExpPattern.name()));
            }
        });
        // initially disable/enable
        m_pattern.setEnabled(m_includeMethod.getStringValue().equals(
            Many2OneCol2NodeModel.IncludeMethod.RegExpPattern.name()));
    }

    /**
     * Standard constructor.
     */
    public Many2OneCol2PMMLNodeModel() {
        this(true);
    }
    ...
    protected PortObject[] execute(final PortObject[] inObjects, final ExecutionContext exec)
        ...
        if (m_pmmlEnabled) {
            ...
            */
            ...
            PMMLMany2OneTranslator trans =
                new PMMLMany2OneTranslator(cellFactory.getAppendedColumnName(), sourceColNames,
        ...
        if (!m_includeMethod.getStringValue().equals(IncludeMethod.RegExpPattern.name())) {
            for (String colName : includes) {
                if (!inDataSpec.getColumnSpec(colName).getType().isCompatible(DoubleValue.class)) {
                    throw new InvalidSettingsException("For selected include method '"
                        + m_includeMethod.getStringValue() + "' only double compatible values are allowed."
