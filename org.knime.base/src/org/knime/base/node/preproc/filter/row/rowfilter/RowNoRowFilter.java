...
public class RowNoRowFilter extends RowFilter {
     ...
     */
    public static final long EOT = -1;

    private static final String CFG_START = "RowRangeStart";

    private static final String CFG_END = "RowRangeEnd";

    private static final String CFG_START_L = "RowRangeStartLong";

    private static final String CFG_END_L = "RowRangeEndLong";
     ...
     * @deprecated use {@link #RowNoRowFilter(long, long, boolean)} instead
     */
    @Deprecated
    public RowNoRowFilter(final int rangeStart, final int rangeEnd, final boolean include) {
        this((long) rangeStart, (long) rangeEnd, include);
    }
     ...
     * @since 3.0
     */
    public RowNoRowFilter(final long rangeStart, final long rangeEnd, final boolean include) {
        if (rangeStart < 0) {
    ...
    public void loadSettingsFrom(final NodeSettingsRO cfg)
        ...
        if (cfg.containsKey(CFG_START_L)) {
            m_start = cfg.getLong(CFG_START_L);
        } else {
            m_start = cfg.getInt(CFG_START);
        }

        if (cfg.containsKey(CFG_END_L)) {
            m_end = cfg.getLong(CFG_END_L);
        } else {
    ...
    protected void saveSettings(final NodeSettingsWO cfg) {
        if (m_start > Integer.MAX_VALUE) {
            cfg.addLong(CFG_START_L, m_start);
        } else {
            cfg.addInt(CFG_START, (int) m_start);
        }
        if (m_end > Integer.MAX_VALUE) {
            cfg.addLong(CFG_END_L, m_end);
        } else {
            cfg.addInt(CFG_END, (int) m_end);
        }
