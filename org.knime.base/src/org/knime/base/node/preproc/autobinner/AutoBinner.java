...
public class AutoBinner {
     ...
     *             when settings are not consistent
     */
     ...
     * @throws Exception
     */
    public PMMLPreprocDiscretize execute(final BufferedDataTable data,
	...
	if (m_settings.getMethod().equals(Method.fixedNumber)) {
		...
		exec.setMessage("Calculating quantiles (column \"" + target
			+ "\")");
		ExecutionContext colSortContext = exec
			.createSubExecutionContext(0.7 / colCount);
		ExecutionContext colCalcContext = exec
			.createSubExecutionContext(0.3 / colCount);
		ColumnRearranger singleRearranger = new ColumnRearranger(spec);
		singleRearranger.keepOnly(target);
		BufferedDataTable singleColSorted = colSortContext
			.createColumnRearrangeTable(data, singleRearranger,
				colSortContext);
		...
		double[] edges = createEdgesFromQuantiles(
			sorted.getBufferedDataTable(), colCalcContext,
	...
	for (String target : m_settings.getTargetColumn()) {
	    String binnedCol = m_settings.getReplaceColumn() ? target : target
		    + " [Binned]";
	    names.add(binnedCol);
	    discretize.put(binnedCol,
		    new PMMLDiscretize(target, binMap.get(target)));
	}
	...
	for (String target : m_settings.getTargetColumn()) {
		...
		bins.add(new PMMLDiscretizeBin(binNames[0], Arrays
			.asList(new PMMLInterval(edges[0], edges[1],
				Closure.closedClosed))));
		for (int i = 1; i < binNames.length; i++) {
		    bins.add(new PMMLDiscretizeBin(binNames[i], Arrays
			    .asList(new PMMLInterval(edges[i], edges[i + 1],
				    Closure.openClosed))));
		}
	...
	for (String colName : recalcValuesFor) {
	    DataColumnSpec colSpec = data.getDataTableSpec().getColumnSpec(
		    colName);
	    if (!colSpec.getType().isCompatible(DoubleValue.class)) {
	...
	DataTableSpec spec = new DataTableSpec(
		newColSpecList.toArray(new DataColumnSpec[0]));
	BufferedDataTable newDataTable = exec.createSpecReplacerTable(data,
		spec);
	return newDataTable;
    }
    ...
    /**
     * @param spec
     *            The <code>DataTableSpec</code> of the input table.
     * @return The spec of the output.
     * @throws InvalidSettingsException
     *             If settings and spec given in the constructor are invalid.
     */
	...
	private static final NumberFormat DEFAULT_FORMAT = NumberFormat
		.getNumberInstance(Locale.US);

	/**
	 * Formats the double to a string. It will use the following either the
	 * format <code>0.00E0</code> for numbers less than 0.0001 or the
