 ...
 * @since 2.10
 */
public class DBSortItem extends JPanel {
     ...
     */
    DBSortItem(final int id, final Vector<DataColumnSpec> values, final String columnName, final boolean sortOrder) {
        this(id, values, new DataColumnSpecCreator(columnName, DataType.getType(DataCell.class)).createSpec(),
            sortOrder);
     ...
     * @param sortOrder the sort
     */
    DBSortItem(final int id, final Vector<DataColumnSpec> values, final DataColumnSpec selected,
        final boolean sortOrder) {
        m_id = id;
        m_combovalues = values;

        super.setLayout(new BoxLayout(this, BoxLayout.X_AXIS));
        m_combo = new JComboBox<DataColumnSpec>(m_combovalues);
        ...
        Border cborder = BorderFactory.createCompoundBorder(outsideborder, insideborder);
        setBorder(cborder);
        if (sortOrder) {
