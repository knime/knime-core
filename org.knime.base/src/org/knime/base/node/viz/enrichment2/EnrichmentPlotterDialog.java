 ...
 */
class EnrichmentPlotterDialog extends NodeDialogPane {
    /**
    ...
    private class MyListModel extends AbstractListModel {
        ...
        private static final long serialVersionUID = -6897205421185376318L;
        ...
        public void addCurve() {
            ...
            if (sort.equals(hit)) {
                JOptionPane.showMessageDialog(getPanel().getParent(), "Please select two different columns.",
                    "Wrong selection", JOptionPane.WARNING_MESSAGE);
                return;
            }

            m_settings.addCurve(sort, hit, m_sortDescending.isSelected());
            fireIntervalAdded(this, m_settings.getCurveCount() - 1, m_settings.getCurveCount() - 1);
    ...
    private EnrichmentPlotterSettings m_settings = new EnrichmentPlotterSettings();

    @SuppressWarnings("unchecked")
    private final ColumnSelectionComboxBox m_sortColumn = new ColumnSelectionComboxBox((Border)null, DoubleValue.class);

    @SuppressWarnings("unchecked")
    private final ColumnSelectionComboxBox m_hitColumn = new ColumnSelectionComboxBox((Border)null, DataValue.class);

    private final JCheckBox m_sortDescending = new JCheckBox("Sort descending");

    private final JRadioButton m_plotSum = new JRadioButton("Plot sum of hit values");

    private final JRadioButton m_plotHits = new JRadioButton("Plot hits ");

    private final JRadioButton m_plotClusters = new JRadioButton("Plot discovered clusters");

    private final JFormattedTextField m_hitThreshold = new JFormattedTextField(new DecimalFormat("###0.0##"));

    private final JSpinner m_minClusterMembers = new JSpinner(new SpinnerNumberModel(1, 1, 1000, 1));

    private final JLabel m_hitClusterLabel = new JLabel("Activity column");

    private final JTextField m_fractionSizes = new JTextField();
    ...
    public EnrichmentPlotterDialog() {
        ...
        p.add(new JLabel("Fraction sizes (in %)"), c);
        c.insets = new Insets(2, 2, 2, 2);
        c.gridy++;
        p.add(m_fractionSizes, c);
    ...
    protected void loadSettingsFrom(final NodeSettingsRO settings, final DataTableSpec[] specs)
        ...
        m_plotClusters.setSelected(m_settings.plotMode().equals(PlotMode.PlotClusters));
        m_hitThreshold.setEnabled(m_settings.plotMode().equals(PlotMode.PlotHits));
        m_minClusterMembers.setEnabled(m_settings.plotMode().equals(PlotMode.PlotClusters));

        m_hitThreshold.setText(Double.toString(m_settings.hitThreshold()));
        m_minClusterMembers.setValue(m_settings.minClusterMembers());
        m_fractionSizes.setText(m_settings.getFractionSizesAsString());
    }
    ...
    protected void saveSettingsTo(final NodeSettingsWO settings) throws InvalidSettingsException {
        ...
        m_settings.hitThreshold(((Number)m_hitThreshold.getValue()).doubleValue());
        m_settings.minClusterMembers(((Number)m_minClusterMembers.getValue()).intValue());
        m_settings.setFractionSizesFromString(m_fractionSizes.getText());
