 ...
 *
 * Adapted to no longer use Jama.Matrix.
 *
 * @author Heiko Hofer
...
final class LogisticRegressionContent {
     ...
     * @param factorList the factors (nominal parameters)
     * @param covariateList the covariates (numeric parameters)
     * @param vectorLengths the length of vector columns
     * @param targetReferenceCategory the target reference category, if not set it is the last category
     * @param sortTargetCategories true when target categories should be sorted
     * @param sortFactorsCategories true when categories of nominal data in the include list should be sorted
     * @param beta the estimated regression factors
    ...
    private static final String CFG_VECTOR_NAMES = "vector_column_names";
    private static final String CFG_VECTOR_LENGTHS = "vector_column_lengths";
    ...
    void save(final ModelContentWO parContent) {
        ...
        parContent.addStringArray(CFG_VECTOR_NAMES,
            m_vectorLengths.entrySet().stream().map(e -> e.getKey()).toArray(String[]::new));
        parContent.addIntArray(CFG_VECTOR_LENGTHS,
