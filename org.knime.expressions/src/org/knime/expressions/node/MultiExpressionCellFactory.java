...
import org.knime.expressions.DefaultWrappedFunctionScript;
import org.knime.expressions.ScriptCompilationException;
import org.knime.expressions.ScriptExecutionException;
import org.knime.expressions.FunctionalScript;
import org.knime.expressions.ScriptParseException;
...
public class MultiExpressionCellFactory extends AbstractCellFactory {
	...
	private final FunctionalScript<DataRow, DataCell>[] m_functions;
	...
	public MultiExpressionCellFactory(DataColumnSpec[] inSpec, DataColumnSpec[] originalSpec, String[] expressions,
		...
		DefaultExpressionPreparer expressionParser = new DefaultExpressionPreparer();
		...
		try {
			m_functions = expressionParser.prepareExpressions(expressions, originalSpec, variables, resultTypes);
	...
	public DataCell[] getCells(DataRow row) {
		...
		for (int i = 0; i < m_functions.length; i++) {
			try {
				returnCells[i] = ((DefaultWrappedFunctionScript) m_functions[i]).apply(row, m_rowindex, m_rowCount);
