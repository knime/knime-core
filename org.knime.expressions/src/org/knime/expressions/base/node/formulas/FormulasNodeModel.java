...
import org.knime.expressions.base.node.MultiExpressionCellFactory;
...
public class FormulasNodeModel extends NodeModel {
	...
	protected BufferedDataTable[] execute(BufferedDataTable[] inData, ExecutionContext exec) throws Exception {
		...
		final ColumnRearranger rearranger = createColumnRearranger(inData[0].getSpec(), inData[0].size(), exec);
	...
	private ColumnRearranger createColumnRearranger(DataTableSpec inSpec, long rowCount, ExecutionContext exec) {
		...
		LinkedList<DataType> returnTypes = new LinkedList<>();
		...
		if (!expressionList.isEmpty()) {
			...
			DataType[] typeArray = new DataType[returnTypes.size()];

			expressionList.toArray(expressionArray);
			specList.toArray(specArray);
			returnTypes.toArray(typeArray);
