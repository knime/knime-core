 ...
 */
public class TWorkflowServiceToThrift implements WorkflowService {

	private final TWorkflowService m_service;
	
	public TWorkflowServiceToThrift(final TWorkflowService service) {
		m_service = service;
	}

	@Override
 	public WorkflowEnt getWorkflow(final EntityID id) {
   	 	 	  		return new TWorkflowEntFromThrift(m_service.TgetWorkflow(new TEntityIDToThrift(id)));
  	}
	
	@Override
 	public void updateWorkflow(final WorkflowEnt wf) {
   	 	 	   		m_service.TupdateWorkflow(new TWorkflowEntToThrift(wf));
  	}
	
	@Override
 	public List<EntityID> getAllWorkflows() {
    		return m_service.TgetAllWorkflows().stream().map(e -> new TEntityIDFromThrift(e)).collect(Collectors.toList());
  	}
