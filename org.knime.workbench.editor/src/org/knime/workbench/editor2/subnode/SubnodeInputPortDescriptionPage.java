 ...
 */
public class SubnodeInputPortDescriptionPage extends WizardPage {

    private static final String DESCRIPTION = "Provide descriptions for the input ports";
     ...
     */
    public SubnodeInputPortDescriptionPage(final String title) {
        super(title);
    ...
    public void setSubNode(final SubNodeContainer subNode) {
        m_subNode = subNode;
        m_descriptions = new String[m_subNode.getNrInPorts() - 1];
        m_names = new String[m_descriptions.length];
        String[] inDescriptions = m_subNode.getInPortDescriptions();
        for (int i = 0; i < m_descriptions.length; i++) {
            m_descriptions[i] = i < inDescriptions.length ? inDescriptions[i] : "";
            m_names[i] = m_subNode.getInPort(i + 1).getPortName();
        }
    }
