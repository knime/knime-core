 ...
 */
public class WorkflowEntToRS {

	private final WorkflowEnt m_e;
	
	public WorkflowEntToRS(final WorkflowEnt e) {
		m_e = e;
	}

	@JsonProperty("Nodes")
    public Map<String, NodeEntToRS> getNodes() {
        	//TODO support non-primitive keys
    	Map<String, NodeEntToRS> res = new HashMap<>();
        m_e.getNodes().entrySet().stream().forEach(e -> res.put(e.getKey(), new NodeEntToRS(e.getValue())));
        return res;
        }
    
	@JsonProperty("Connections")
    public List<ConnectionEntToRS> getConnections() {
        	return m_e.getConnections().stream().map(l -> new ConnectionEntToRS(l)).collect(Collectors.toList());
        }
    
	@JsonProperty("MetaInPorts")
    public List<MetaPortEntToRS> getMetaInPorts() {
        	return m_e.getMetaInPorts().stream().map(l -> new MetaPortEntToRS(l)).collect(Collectors.toList());
        }
    
	@JsonProperty("MetaOutPorts")
    public List<MetaPortEntToRS> getMetaOutPorts() {
        	return m_e.getMetaOutPorts().stream().map(l -> new MetaPortEntToRS(l)).collect(Collectors.toList());
        }
