 ...
 */
public class TestEntToRS {

	private final TestEnt m_e;
	
	public TestEntToRS(final TestEnt e) {
		m_e = e;
	}

	@JsonProperty("xy")
    public XYEntToRS getxy() {
            return new XYEntToRS(m_e.getxy());
        }
    
	@JsonProperty("xylist")
    public List<XYEntToRS> getxylist() {
        	return m_e.getxylist().stream().map(l -> new XYEntToRS(l)).collect(Collectors.toList());
        }
	...
	@JsonProperty("xymap")
    public Map<String, XYEntToRS> getxymap() {
        	//TODO support non-primitive keys
    	Map<String, XYEntToRS> res = new HashMap<>();
        m_e.getxymap().entrySet().stream().forEach(e -> res.put(e.getKey(), new XYEntToRS(e.getValue())));
        return res;
        }
