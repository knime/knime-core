...
import org.knime.node.v41.Port;
...
/**
 * Implementation of {@link NodeDescription} for node descriptions introduced with 4.1. It uses XMLBeans to extract the
...
public final class NodeDescription41Proxy extends NodeDescription {
     ...
     */
    private NodeDescription41Proxy(final KnimeNodeDocument doc, final boolean validate) {
        m_document = doc;
        setIsDeprecated(m_document.getKnimeNode().getDeprecated());
        if (validate) {
            validate();
    ...
    NodeDescription createUpdatedNodeDescription(final ModifiablePortsConfiguration portsConfiguration) {
        if (portsConfiguration != null) {
            ...
            if (!extendablePortGrps.isEmpty()) {
                final NodeDescription41Proxy modifiedDescription =
                    new NodeDescription41Proxy((KnimeNodeDocument)m_document.copy(), false);
