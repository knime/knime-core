 ...
 */
package org.knime.core.node.webui.internal;
...
/**
 * Implemented by {@link NodeFactory}s to register internal(!) node views. Must not be extended by third-party
 * contributions.
 *
 * Pending API!
 *
 ...
 */
public interface NodeViewFactoryInternal<T extends NodeModel> {
    ...
    static boolean hasNodeView(final NodeContainer nc) {
        return nc instanceof NativeNodeContainer
            && ((NativeNodeContainer)nc).getNode().getFactory() instanceof NodeViewFactoryInternal;
    }
    ...
    static NodeView<NodeModel> createNodeView(final NodeContainer nc) {
        if (nc instanceof NativeNodeContainer) {
            ...
            if (fac instanceof NodeViewFactoryInternal) {
                return ((NodeViewFactoryInternal<NodeModel>)fac).createNodeView();
            }
        }
    ...
    /**
     * Creates a new node view instance. The {@link Page} provided by internal node views actually represents a
     * vue-component (i.e. a js-file) provided at 'runtime' (and <b>not</b> a html file).
     *
     * @return a new node view instance
     */
