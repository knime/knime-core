...
public final class WorkflowLogCloseable implements AutoCloseable {
    ...
    /** Default log file appender, public for access from {@link NodeLoggerConfig}, in this class to access its filter. */
    public static Appender LOG_FILE_APPENDER;

    /**
     * Whether to log in the workflow directory or not.
     */
    public static boolean LOG_IN_WF_DIR;

    /**
     * Whether to log messages without associated directory (i.e. "global" messages) into workflow logs.
     */
    public static boolean LOG_GLOBAL_IN_WF_DIR;

    /**
     * Include node id in log message.
     */
    public static boolean LOG_NODE_ID;

    /**
     * Include workflow dir in log message.
     */
    public static boolean LOG_WF_DIR;

    /**
     * Include job ID (Hub/Server only) in log message.
     */
    public static boolean LOG_JOB_ID;
    /* ======= End of moved fields. */
    ...
    public static void checkLayoutFlags(final Layout layout) {
        if (layout instanceof PatternLayout pl) {
            ...
            WorkflowLogCloseable.LOG_NODE_ID |= conversionPattern.contains("%" + NodeLoggerPatternConstants.NODE_ID);
            WorkflowLogCloseable.LOG_NODE_ID |= conversionPattern.contains("%" + NodeLoggerPatternConstants.NODE_NAME);
            WorkflowLogCloseable.LOG_NODE_ID |= conversionPattern.contains("%" + NodeLoggerPatternConstants.QUALIFIER);
            ...
            WorkflowLogCloseable.LOG_WF_DIR |= conversionPattern.contains("%" + NodeLoggerPatternConstants.WORKFLOW_DIR);
            if (WorkflowLogCloseable.LOG_WF_DIR) {
            ...
            WorkflowLogCloseable.LOG_JOB_ID |= conversionPattern.contains("%" + NodeLoggerPatternConstants.JOB_ID);
            if (WorkflowLogCloseable.LOG_JOB_ID) {
    ...
    static LogfileAppender createWorkflowLogAppender(final String appenderName, final Path workflowDirPath) {
        ...
        final var mainFilter = LOG_FILE_APPENDER.getFilter();
        final var workflowDirFilter = new Filter() {
            @Override
            public int decide(final LoggingEvent event) {
                // DENY cuts off filtering early and not consult the next filter
                if (!(LOG_IN_WF_DIR && event.getMessage() instanceof KNIMELogMessage kmsg)) {
                ...
                if (!(messageWfDir == null ? LOG_GLOBAL_IN_WF_DIR : workflowDir.equals(messageWfDir))) {
                    return Filter.DENY;
                }
