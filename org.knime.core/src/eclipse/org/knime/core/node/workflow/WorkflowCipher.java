...
final class WorkflowCipher implements Cloneable {
    ...
    private enum CipherVersion {
        /** Version 3 is used in KNIME AP 4.6.4--present, see AP-19646. */
        VERSION_3("version_3") {
            @Override
            final Optional<byte[]> extractKey(final byte[] prefix, final byte[] versionedKey) {
                // identical to extractKey of VERSION_2
            ...
            byte[] packageKey(final byte[] prefix, final byte[] rawKey) {
                // identical to packageKey of VERSION_2
        ...
        /** Version 2 is used in KNIME AP 4.6.2--4.6.3, see AP-19479. */
        VERSION_2("version_2") {
            @Override
            final Optional<byte[]> extractKey(final byte[] prefix, final byte[] versionedKey) {
                final var extracted = stripPrefix(prefix, versionedKey);
    ...
    /** Random key generated by a (AES) {@link KeyGenerator}. */
    private final SecretKey m_secretKey;

    private final String m_passwordHint;

    /** The (SHA1) hash of the plaintext password. */
    ...
    /**
     * Prompt for password until {@link #hashPassword(entered password)} equals this {@link WorkflowCipher}'s password
     * digest or the user cancels.
     *
    ...
    boolean unlock(final WorkflowCipherPrompt prompt) {
        ...
        LOGGER.debug("Prompting user for " + (isPromptForComponent ? "component" : "metanode") + " password, hint \""
            + hint + "\"");
        try {
            // check if the hash of the entered password equals the password digest stored in the WorkflowCipher
