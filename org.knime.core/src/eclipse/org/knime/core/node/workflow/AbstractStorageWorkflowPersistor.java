 ...
 */
public abstract class AbstractStorageWorkflowPersistor implements WorkflowPersistor, TemplateNodeContainerPersistor {
    ...
    static final LoadVersion VERSION_LATEST = LoadVersion.V3080;
    ...
    private final AbstractStorageNodeContainerMetaPersistor m_metaPersistor;
     ...
     */
    AbstractStorageWorkflowPersistor(final AbstractStorageNodeContainerMetaPersistor metaPersistor,
    ...
    public final LoadVersion getLoadVersion() {
        return m_metaPersistor.getLoadVersion();
    }
    ...
    /** {@inheritDoc} */
    @Override
    public AbstractStorageNodeContainerMetaPersistor getMetaPersistor() {
    ...
    abstract NodeSettingsRO readWorkflowSettings() throws IOException;

    /**
     * @return the name of the workflow, usually the name of the directory where the workflow is loaded from. Used for
     *         error messaging.
     */
    abstract String getWorkflowSource();
    ...
    public void preLoadNodeContainer(final WorkflowPersistor parentPersistor, final NodeSettingsRO parentSettings,
        ...
        NodeSettingsRO subWFSettings = readWorkflowSettings();
        ...
        if (hasPorts && m_isProject) {
            throw new InvalidSettingsException(String.format("Workflow \"%s\""
                + " is not a project as it has ports (%d in, %d out)", getWorkflowSource(), inPortCount,
                outPortCount));
        }
    ...
    public void loadNodeContainer(final Map<Integer, BufferedDataTable> tblRep, final ExecutionMonitor exec,
        ...
        final ReferencedFile workflowDirRef = getMetaPersistor().getNodeContainerDirectory();
        /* Load nodes */
        for (String nodeKey : nodes.keySet()) {
            ...
            try {
                persistor = createNodeContainerPersitorLoad(nodeSetting, nodeType);
            } catch (InvalidSettingsException e) {
                String error =
            ...
            try {
                ...
                if (e instanceof NodeFactoryUnknownException) {
                    ...
                    setDirtyAfterLoad();
                    failingNodeIDSet.add(nodeIDSuffix);
                    onAddedToFailingNodesList(nodeIDSuffix, nodeSetting);
                    continue;
                }
            }
    ...
    abstract FromFileNodeContainerPersistor createNodeContainerPersitorLoad(
        final NodeSettingsRO nodeSetting, final NodeType nodeType) throws InvalidSettingsException;
