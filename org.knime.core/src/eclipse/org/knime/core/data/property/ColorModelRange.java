...
public final class ColorModelRange implements ColorModel {
    ...
    public static ColorModelRange load(final ConfigRO config) throws InvalidSettingsException {
        ...
        var stopValues = new double[0];
        var stopColorsHex = new String[0];
        if (gradient == ColorGradient.CUSTOM) {
            stopValues = config.getDoubleArray(CFG_STOP_VALUES);
            stopColorsHex = config.getStringArray(CFG_STOP_COLORS_HEX);
        } else if (!stopValuesArePercentages) {
            stopValues = config.getDoubleArray(CFG_STOP_VALUES);
            return new ColorModelRange(specialColors, gradient).applyToDomain(stopValues[0], stopValues[1]);
        }
