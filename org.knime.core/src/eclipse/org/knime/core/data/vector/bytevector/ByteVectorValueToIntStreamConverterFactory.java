...
/**
 * Converter that translates a {@link ByteVectorValue} to an {@link IntStream} for use in the Java Snippet,
 * for instance. The values in the stream are in [0-255]. This class is registered via extension point.
 *
 * @author Bernd Wiswedel, KNIME.com, Zurich, Switzerland
 * @since 3.4
 */
public final class ByteVectorValueToIntStreamConverterFactory
extends SimpleDataCellToJavaConverterFactory<ByteVectorValue, IntStream> {

    /** Calls super constructor and sets all fields (including the conversion logic). */
    public ByteVectorValueToIntStreamConverterFactory() {
        super(ByteVectorValue.class, IntStream.class, v -> {
            return IntStream.generate(new IntSupplier() {
                long m_index = 0L;
                @Override
                public int getAsInt() {
                    return v.get(m_index++);
                }
            }).limit(v.length());
