...
/**
 * Interface for {@link CellValueProducer producers} of Java primitive {@code char} values.
 *
 ...
 */
public interface CharCellValueProducer<S extends Source<?>, PP extends Source.ProducerParameters<S>>
    extends PrimitiveCellValueProducer<S, Character, PP> {

    /**
     * Reads an {@code char} value from the given source using the given parameters.
     *
     ...
     */
    char produceCharCellValue(S source, PP params) throws MappingException;

    @Override
    default Character produceCellValue(final S source, final PP params) throws MappingException {
        if (producesMissingCellValue(source, params)) {
            return null;
        } else {
            return produceCharCellValue(source, params);
        }
    }
}