...
/**
 * Interface for {@link CellValueProducer producers} of Java primitive {@code short} values.
 *
 ...
 */
public interface ShortCellValueProducer<S extends Source<?>, PP extends Source.ProducerParameters<S>>
    extends PrimitiveCellValueProducer<S, Short, PP> {

    /**
     * Reads an {@code short} value from the given source using the given parameters.
     *
     ...
     */
    short produceShortCellValue(S source, PP params) throws MappingException;

    @Override
    default Short produceCellValue(final S source, final PP params) throws MappingException {
        if (producesMissingCellValue(source, params)) {
            return null;
        } else {
            return produceShortCellValue(source, params);
        }
    }
}