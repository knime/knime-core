...
/**
 * Interface for {@link CellValueProducer producers} of Java primitive {@code long} values.
 *
 ...
 */
public interface LongCellValueProducer<S extends Source<?>, PP extends Source.ProducerParameters<S>>
    extends PrimitiveCellValueProducer<S, Long, PP> {

    /**
     * Reads an {@code long} value from the given source using the given parameters.
     *
     ...
     */
    long produceLongCellValue(S source, PP params) throws MappingException;

    @Override
    default Long produceCellValue(final S source, final PP params) throws MappingException {
        if (producesMissingCellValue(source, params)) {
            return null;
        } else {
            return produceLongCellValue(source, params);
        }
    }
}