...
/**
 * Interface for {@link CellValueProducer producers} of Java primitive {@code byte} values.
 *
 ...
 */
public interface ByteCellValueProducer<S extends Source<?>, PP extends Source.ProducerParameters<S>>
    extends PrimitiveCellValueProducer<S, Byte, PP> {

    /**
     * Reads an {@code byte} value from the given source using the given parameters.
     *
     ...
     */
    byte produceByteCellValue(S source, PP params) throws MappingException;

    @Override
    default Byte produceCellValue(final S source, final PP params) throws MappingException {
        if (producesMissingCellValue(source, params)) {
            return null;
        } else {
            return produceByteCellValue(source, params);
        }
    }
}