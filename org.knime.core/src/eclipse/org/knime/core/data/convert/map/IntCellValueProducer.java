...
/**
 * Interface for {@link CellValueProducer producers} of Java primitive {@code int} values.
 *
 ...
 */
public interface IntCellValueProducer<S extends Source<?>, PP extends Source.ProducerParameters<S>>
    extends PrimitiveCellValueProducer<S, Integer, PP> {

    /**
     * Reads an {@code int} value from the given source using the given parameters.
     *
     ...
     */
    int produceIntCellValue(S source, PP params) throws MappingException;

    @Override
    default Integer produceCellValue(final S source, final PP params) throws MappingException {
        if (producesMissingCellValue(source, params)) {
            return null;
        } else {
            return produceIntCellValue(source, params);
        }
    }
}
