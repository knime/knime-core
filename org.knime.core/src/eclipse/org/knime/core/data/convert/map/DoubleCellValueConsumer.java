...
/**
 * Interface for {@link CellValueConsumer consumers} of Java primitive {@code double} values.
 *
 ...
 */
public interface DoubleCellValueConsumer<D extends Destination<?>, CP extends Destination.ConsumerParameters<D>>
    extends PrimitiveCellValueConsumer<D, Double, CP> {

    /**
     * Writes the given {@code double} value to the given destination using the given parameters.
     *
     ...
     */
    void consumeDoubleCellValue(D destination, double value, CP params) throws MappingException;

    @Override
    default void consumeCellValue(final D destination, final Double value, final CP consumerParams)
        throws MappingException {
        if (value != null) {
            consumeDoubleCellValue(destination, value, consumerParams);
        } else {
