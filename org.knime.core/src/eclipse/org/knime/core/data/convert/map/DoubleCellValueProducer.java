...
public interface DoubleCellValueProducer<S extends Source<?>, PP extends Source.ProducerParameters<S>>
     ...
     */
    double produceDoubleCellValue(S source, PP params) throws MappingException;

    @Override
    default Double produceCellValue(final S source, final PP params) throws MappingException {
        if (producesMissingCellValue(source, params)) {
            return null;
        } else {
            return produceDoubleCellValue(source, params);
        }
    }
}