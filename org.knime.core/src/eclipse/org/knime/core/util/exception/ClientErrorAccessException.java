 ...
 * @since 5.0
 */
public class ClientErrorAccessException extends HttpResourceAccessException { // NOSONAR
     ...
     * @param statusCode Status code of the response.
     */
    public ClientErrorAccessException(final String message, final Throwable cause, final int statusCode) {
        super(message != null ? message : cause.getMessage(), cause, statusCode);
        validateStatus(statusCode);
    }

    private static void validateStatus(final int statusCode) {
        CheckUtils.checkArgument(400 <= statusCode && statusCode <= 499,
            "Status codes for client error exceptions must be from the 4XX family, got: %d", statusCode);
    }
}
