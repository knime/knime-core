...
final class PMMLDiscretizeBinContentHandler extends PMMLContentHandler {
    private static final NodeLogger LOGGER = NodeLogger.getLogger(PMMLDiscretizeBinContentHandler.class);
     ...
     */
    PMMLDiscretizeBinContentHandler(final Stack<PMMLContentHandler> contentHandlerStack, final Attributes atts,
        final List<PMMLDiscretizeBin> list) {
        m_contentHandlerStack = contentHandlerStack;
        m_list = list;
        m_binValue = atts.getValue("binValue");
        m_intervalList = new ArrayList<>();
    }
    ...
    public void startElement(final String uri, final String localName, final String name, final Attributes atts)
        throws SAXException {
        if ("Extension".equals(name)) {
            LOGGER.debug("Skipping unknown extension in DiscretizeBin: " + name);
        } else if ("Interval".equals(name)) {
            m_contentHandlerStack.push(new PMMLIntervalContentHandler(m_contentHandlerStack, atts, m_intervalList));
        } else {
            throw new SAXException("Unknown xml element in DiscretizeBin: " + name);
        }
    }
