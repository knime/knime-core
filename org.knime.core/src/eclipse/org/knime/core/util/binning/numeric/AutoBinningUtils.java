...
public class AutoBinningUtils {
    ...
    static IntPair findStreakEndpoints(final List<Double> values, final int beginning, final boolean ceil) {
        ...
        while (leftIndex > 0 && maybeCeilEquals.test(streakValue, values.get(leftIndex - 1).doubleValue())) {
            leftIndex--;
        }

        int rightIndex = beginning;
        while (rightIndex < values.size() - 1
            && maybeCeilEquals.test(values.get(rightIndex + 1).doubleValue(), streakValue)) {
            rightIndex++;
        }
    ...
    static List<BinBoundary> createEdgesForEqualCount( //
        ...
        for (int i = 1; i < edges.length - 1; i++) {
            ...
            if (guessForNextBoundIndex < tableValues.size()) {
                ...
                var edge_ = maybeCeil.applyAsDouble(tableValues.get(guessForNextBoundIndex));
                int lowerIndex_ = guessForNextBoundIndex;
                while (lowerIndex_ >= lowestPossibleNextBoundIndex
                    && !(edge_ > maybeCeil.applyAsDouble(tableValues.get(lowerIndex_)))) {
                    lowerIndex_--;
                }
                // get higher index
                int higherIndex_ = guessForNextBoundIndex;
                while (higherIndex_ < tableValues.size() - 1
                    && !(maybeCeil.applyAsDouble(tableValues.get(higherIndex_ + 1)) > edge_)) {
                    higherIndex_++;
                }

                if (lowerIndex_ != lowerIndex || higherIndex_ != higherIndex) {
                    // some errors should be logged
                    NodeLogger.getLogger(AutoBinningUtils.class)
                        .warn("Old algorithm gave different values to new algorithm for bin boundaries.\n\n"
                            + "Old lower index: " + lowerIndex_ + ", old higher index: " + higherIndex_ + "\n"
                            + "New lower index: " + lowerIndex + ", new higher index: " + higherIndex
                            + "\n\nThis is a bug.");
                } else {
                    NodeLogger.getLogger(AutoBinningUtils.class)
                        .info("Old algorithm and new algorithm gave same values.\n\n" + "Lower index: " + lowerIndex
                            + ", higher index: " + higherIndex + "\n\nThis is expected.");
                }
