...
public class ThreadPool {
        ...
        if (!b) {
            ...
            try {
                ...
                try {
                     ...
                     * to the calling method. This means that the caller can opt to swallow the interrupted exception
                     * in order to keep the thread running and thus keeping the pool oversubscribed. Once the current
                     * task is finished, the thread will be put back into the available workers but only actually used
                     * if the pool is not saturated anymore. This is exactly the behavior as before the
                     * "re-acquire slot after call" change in AP-24224.
                     */
    ...
    private void reacquireSlot() throws InterruptedException {
        synchronized (m_runningWorkers) {
