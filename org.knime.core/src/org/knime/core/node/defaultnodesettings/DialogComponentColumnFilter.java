...
public class DialogComponentColumnFilter extends DialogComponent {
    ...
    /* Include list. */
    private JList m_inclList;

    /* Include model. */
    private DefaultListModel m_inclMdl;

    /* Exclude list. */
    private JList m_exclList;

    /* Exclude model. */
    private DefaultListModel m_exclMdl;
     ...
     */
    public DialogComponentColumnFilter(final SettingsModelStringArray model,
            final String label, final int inPortIndex) {
        this(model, label, inPortIndex, true);
    }

    /**
     * Creates a new filter column panel with three component which are the
     * include list, button panel to shift elements between the two lists, and
     * the exclude list. Depending on the <code>excluded</code> flag, either
     * the excluded (set true), or included list will be stored in the model and
     * the settings object.
     * 
     * @param model a string array model that stores the value
     * @param label description of this panel
     ...
     *            settings model, false if the list of included columns should
     *            be stored.
     * @see #update(DataTableSpec, Set, boolean)
     */
    public DialogComponentColumnFilter(final SettingsModelStringArray model,
            final String label, final int inPortIndex, final boolean excluded) {
        super(model);
        ...
        final JPanel buttonPan = createButtonPanel();
        // include list
        final JPanel includePanel = createIncludePanel();

        // exclude list
        final JPanel excludePanel = createExcludePanel();
        ...
        getModel().addChangeListener(new ChangeListener() {
            public void stateChanged(final ChangeEvent e) {
                update(m_lastSpec, new HashSet<String>(Arrays
                        .asList(((SettingsModelStringArray)getModel())
                                .getStringArrayValue())), m_storeExcluded);
            }
        });
    ...
    /**
     * @return a JPanel containing the include list in a JList
     */
    ...
    /**
     * @return a JPanel with the add/remove buttons to shift columns from the
     *         include to the exclude list and vice versa.
     */
    ...
    /**
     * @return a JPanel containing the exclude list in a JList.
     */
    ...
    void validateStettingsBeforeSave() throws InvalidSettingsException {
        ...
        ((SettingsModelStringArray)getModel()).setStringArrayValue(colList
                .toArray(new String[colList.size()]));
    ...
    private static int searchInList(final JList list, final String str,
        ...
        String prefix = str;
        if (prefix == null) {
            throw new IllegalArgumentException("Parameter str must "
                    + "not be null");
        }
