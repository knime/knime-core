...
public final class Node {
     ...
     */
    // TODO remove this flag from the Node class
    protected boolean m_isCurrentlySaved = false;
...
// TODO (tg,po)
    ...
    public void loadSettingsFrom(final NodeSettingsRO settings)
        ...
        if (getNrOutPorts() > 0) {
            m_outDataPortsMemoryPolicy = l.getMemoryPolicy() == null
            ? MemoryPolicy.CacheSmallInMemory : l.getMemoryPolicy();
            }
        } 
...
//                notifyStateListeners(new NodeStateChangedEvent.EndExecute());
        ...
        for (int p = 0; p < getNrOutPorts(); p++) {
            if (newOutData[p] instanceof BufferedDataTable) {
                ...
                DataTableSpec portSpec = (DataTableSpec)(
                        m_outPorts[p].getPortObjectSpec());
    ...
    /**
     * Removes the node directory and all data inside.
     */
...
//                                + "are configured and/or executed.");
     ...
     * @throws ArrayIndexOutOfBoundsException If the view index is out of range.
     */
    public GenericNodeView<?> getView(
            final int viewIndex, final String title) {
        GenericNodeView<?> view =
            m_factory.createNodeView(viewIndex, m_model);
...
//    }
    ...
    void saveInternals(final File internDir, final ExecutionMonitor exec)
        ...
        m_logger.assertLog(m_model.isExecuted(), "Can't save internals, not executed");
            ...
            String errorMessage = "Unable to write directory: "
                + internDir.getAbsolutePath();
...
//    }
    ...
    public void setScopeContextStackContainer(
            final ScopeObjectStack scsc) {
     ...
     */
        ...
        static SettingsLoaderAndWriter load(final NodeSettingsRO settings)
            ...
            if (settings.containsKey(CFG_MISC_SETTINGS) &&
                    settings.getNodeSettings(CFG_MISC_SETTINGS).containsKey(Node.CFG_MEMORY_POLICY)) {
                NodeSettingsRO sub = settings.getNodeSettings(CFG_MISC_SETTINGS);
                String memoryPolicy = sub.getString(Node.CFG_MEMORY_POLICY,
                ...
                try {
                    ...
                    throw new InvalidSettingsException("Invalid memory policy: "
                            + memoryPolicy);
                }
        ...
        void save(final NodeSettingsWO settings) {
            if (m_memoryPolicy != null) {
                NodeSettingsWO sub = settings.addNodeSettings(CFG_MISC_SETTINGS);
