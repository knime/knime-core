...
public class AutoLayoutAction extends AbstractNodeAction {
    ...
    public static final String ID = "knime.action.autolayout";

    /**
     * @param editor The workflow editor
     */
    public AutoLayoutAction(final WorkflowEditor editor) {
        super(editor);
    }
    ...
    public synchronized void runOnNodes(final NodeContainerEditPart[] parts) {
        ...
        ScrollingGraphicalViewer provider =
                (ScrollingGraphicalViewer)getEditor().getEditorSite()
                        .getSelectionProvider();
        if (provider == null) {
            return;
        }

        ArrayList<UINodeContainer> nodes = null;
        if (parts != null && parts.length > 0) {
            nodes = new ArrayList<UINodeContainer>(parts.length);
