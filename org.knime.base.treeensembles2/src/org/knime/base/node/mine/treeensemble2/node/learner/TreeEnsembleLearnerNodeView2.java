...
import org.knime.base.node.mine.treeensemble2.node.view.classification.RandomForestClassificationTreeGraphView;
...
public final class TreeEnsembleLearnerNodeView2<MODEL extends NodeModel & ViewContentProvider>
    ...
    private static final NodeLogger LOGGER = NodeLogger.getLogger(TreeEnsembleLearnerNodeView2.class);
    ...
    private RandomForestAbstractTreeGraphView m_graph;
     ...
     */
    public TreeEnsembleLearnerNodeView2(final MODEL model) {
        super(model);
        ...
        m_graph = new RandomForestClassificationTreeGraphView(null, true, true);
        ...
        m_popup.add(new ExpandBranchAction<AbstractTreeNode>(m_graph));
        m_popup.add(new CollapseBranchAction<AbstractTreeNode>(m_graph));

        m_graph.getView().addMouseListener(new MouseAdapter() {
            private void showPopup(final MouseEvent e) {
                AbstractTreeNode node = m_graph.nodeAtPoint(e.getPoint());
                if (null != node) {
...
//        AbstractTreeNode selected = m_graph.getSelected();
//        if (selected == null) {
//            return;
//        }
...
//    }
    ...
    private final class UpdateTreeWorker extends SwingWorkerWithContext<AbstractTreeNode, Void> {
        ...
        /** {@inheritDoc} */
        @Override
        protected AbstractTreeNode doInBackgroundWithContext() throws Exception {
            if (m_model == null || m_index < 0 || m_index >= m_model.getNrModels()) {
                return null;
            }
            final AbstractTreeModel tree = m_model.getTreeModel(m_index);
