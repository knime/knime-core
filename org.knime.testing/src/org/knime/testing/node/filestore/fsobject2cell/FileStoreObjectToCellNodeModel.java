...
final class FileStoreObjectToCellNodeModel extends NodeModel {

    private final SettingsModelString m_colNameModel = createColNameModel();

    FileStoreObjectToCellNodeModel() {
        super(new PortType[] {LargeFileStorePortObject.TYPE}, new PortType[] {BufferedDataTable.TYPE});
    }
    ...
    private final DataTableSpec createOutputSpec() throws InvalidSettingsException {
        String colName = m_colNameModel.getStringValue();
        if (colName == null || colName.length() == 0) {
            throw new InvalidSettingsException("No column name provided");
        }
        return new DataTableSpec(new DataColumnSpecCreator(colName, LargeFileStoreCell.TYPE).createSpec());
    }
    ...
    protected void saveSettingsTo(final NodeSettingsWO settings) {
        m_colNameModel.saveSettingsTo(settings);
    }

    @Override
    protected void validateSettings(final NodeSettingsRO settings) throws InvalidSettingsException {
        m_colNameModel.validateSettings(settings);
    }

    @Override
    protected void loadValidatedSettingsFrom(final NodeSettingsRO settings) throws InvalidSettingsException {
        m_colNameModel.loadSettingsFrom(settings);
    }
    ...
    static final SettingsModelString createColNameModel() {
        return new SettingsModelString("colName", "large FS store");
    }
